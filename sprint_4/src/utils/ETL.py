...
def create_tables(sql_script_file_path): #prefix file path with r
    try:
        with open(sql_script_file_path, 'r') as sql_file:
            ...
            LOGGER.info("All data tables created successfully.")
    except Exception as e:
        conn.rollback()
        LOGGER.errorr("Table creation failed: %s", e)
        return
...
def load(branches, transactions, products, transaction_items):
    
    # Create the tables before entering our transformed data.

    create_tables(r"C:\Users\GuledM(DE-LON16)\Documents\Group-Project\brews_brothers_DELON16\sprint_3\docker\v.2-SQL-script.sql") #sql script file path
